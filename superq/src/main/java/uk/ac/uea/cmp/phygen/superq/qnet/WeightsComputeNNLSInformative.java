...
public class WeightsComputeNNLSInformative {
    ...
    public static ComputedWeights computeWeights(GroupedQuartetSystem quartetSystem,
...
//**************************************************************************************************************
        ...
        /*
         * try { FileWriter fw = new FileWriter ("matrix.dat");
         *
         * for(int i = 0; i < N * (N - 1) / 2 - N; i++) { for(int j = 0; j < N *
         * (N - 1) / 2 - N; j++) { fw.write (EtE.getElementAt (i, j) + " "); }
         * fw.write ("\n"); } fw.close(); }catch (IOException exception)
         * {System.out.println("Error while writing matrix.dat.");}
         */

        // writing initial.dat
        /*
         * try { FileWriter out = new FileWriter ("initial.dat"); for(int i = 0;
         * i < N * (N - 1) / 2 - N ; i++) { out.write("0\n"); } out.close();
         * }catch(IOException exception){System.out.println("Error while writing
         * initial.dat.");} // end
         *
         * double[] cleaned = new double[N * (N - 1) / 2 - N];
         *
         * try { LineNumberReader ln_reader = new LineNumberReader(new
         * FileReader("initial.dat"));
         *
         * for(int i = 0; i < N * (N - 1) / 2 - N; i++) { cleaned[i] =
         * Integer.parseInt(ln_reader.readLine()); } ln_reader.close(); }catch
         * (IOException exception) {System.out.println("Error while reading in
         * initial.dat");}
         */
