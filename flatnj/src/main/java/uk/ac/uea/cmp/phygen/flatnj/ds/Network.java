...
import uk.ac.uea.cmp.phygen.flatnj.fdraw.Collector;
import uk.ac.uea.cmp.phygen.flatnj.fdraw.DrawFlat;
import uk.ac.uea.cmp.phygen.flatnj.fdraw.Edge;
import uk.ac.uea.cmp.phygen.flatnj.fdraw.Vertex;
    ...
    public Map<Integer,boolean[]> getSplits()
    {
        ...
        while(sIt.hasNext())
        {
            ...
            while(!vv.isEmpty())
            {
                ...
                uk.ac.uea.cmp.phygen.flatnj.fdraw.Label l = v.getLabel();
                if(l != null)
                {
                    String[] labels = l.getName().split("\\s*,\\s*");
