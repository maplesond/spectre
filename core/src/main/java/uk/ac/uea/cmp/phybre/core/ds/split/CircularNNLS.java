    ...
    void runActiveConjugate(int ntax, double[][] d, double[][] W, double[][] x) {
        ...
        while (true) {
            while (true) /* Inner loop: find the next feasible optimum */ {
                ...
                if (collapse_many_negs) { /* Typically, a large number of edges are negative, so on the first
                    ...
                    if (num_neg_indices > 0) {
                        ...
                        runConjugateGrads(ntax, r, w, p, y, W, AtWd, active, x); /* Re-uk.ac.uea.cmp.phybre.core.math.optimise, so that the current x is always optimal */
                    }
                }
            ...
            if (!getMinimizeAIC()) {
                 ...
                 * If we add b_i back in, and just uk.ac.uea.cmp.phybre.core.math.optimise that single variable, then the AIC decreases when
