...
public class LabelPlacementOptimizer4Points implements LabelPlacementOptimizer {
    ...
    @Override
    public void placeLabels(Collection<ViewerLabel> labelSet,
        ...
        ViewerLabel[] labels = new ViewerLabel[n];
    ...
    private boolean hasPointInside(Box box, ViewerLabel[] labels) {
        for (ViewerLabel label : labels) {
            ViewerPoint p = label.p;
    ...
    private int intersectsWithLine(Box box, Collection<Line> lineSet) {
        ...
        while (lineIt.hasNext()) {
            ...
            ViewerPoint p1 = ViewerLabel.intersectionPoint(l.p1.getX(), l.p1.getY(), l.p2.getX(), l.p2.getY(),
                    box.x2, box.y2, box.x2, box.y1);
            ViewerPoint p2 = ViewerLabel.intersectionPoint(l.p1.getX(), l.p1.getY(), l.p2.getX(), l.p2.getY(),
                    box.x1, box.y2, box.x2, box.y2);
            ViewerPoint p3 = ViewerLabel.intersectionPoint(l.p1.getX(), l.p1.getY(), l.p2.getX(), l.p2.getY(),
                    box.x1, box.y1, box.x1, box.y2);
            ViewerPoint p4 = ViewerLabel.intersectionPoint(l.p1.getX(), l.p1.getY(), l.p2.getX(), l.p2.getY(),
    ...
    private void computeInitialPositions(int n, Collection<ViewerLabel> labelSet, ViewerLabel[] labels, Box[] candidatePositions) {
        Iterator<ViewerLabel> labelIt = labelSet.iterator();
        for (int i = 0; i < n; i++) {
    ...
    private Map<Integer, Box> findTrivialPositions(int n,
        ...
        for (int i = 0; i < n; i++) {
            ViewerLabel l = labels[i];
            ViewerPoint p = l.p;
            Double minScore = null;
            Integer bestPosition = null;
            for (int j = 0; j < pn; j++) {
                if (size(intersectionGraph[i * pn + preferedQuarters[p.quarterNo][j]]) == 0) {
    ...
    private void resolveRemainingPositions(int n, Map<Integer, Box> fixedPositions, Box[] candidatePositions, boolean[][] intersectionGraph, ViewerLabel[] labels) {
        for (int i = 0; i < n; i++) {
    ...
    private void fixPostions(Map<Integer, Box> fixedPositions, ViewerLabel[] labels, boolean all) {
        for (int i = 0; i < labels.length; i++) {
            ViewerLabel l = labels[i];
    ...
    private void estimateNeighborhoods(Collection<ViewerLabel> labelSet) {
        Iterator<ViewerLabel> labelIt = labelSet.iterator();
        List<ViewerLabel> labelsToCheck = new LinkedList();
        while (labelIt.hasNext()) {
            ViewerLabel l = labelIt.next();
            ViewerPoint p = l.p;
            p.neighborhood = new HashSet();
            for (int i = 0; i < labelsToCheck.size(); i++) {
                ViewerLabel l2 = labelsToCheck.get(i);
                ViewerPoint p2 = l2.p;
    ...
    protected static class Box {
        ...
        public Box(double x1, double y1, int width, int height, ViewerLabel l) {
            this.x1 = x1;
            this.y1 = y1;
            this.x2 = x1 + width;
            this.y2 = y1 - height;
        ...
        private ViewerPoint middlePoint() {
            return new ViewerPoint((int) (0.5 * (x1 + x2)), (int) (0.5 * (y1 + y2)));
        }

        private boolean pointInside(ViewerPoint p1) {
